...
class FileUploadUtility {
    ...
    /**
     * Upload and process page image
     */
    public static function uploadPageImage($file) {
        // Validate file
        $validation = self::validateFile($file);
        if (!$validation['valid']) {
            return ['success' => false, 'errors' => $validation['errors']];
        ...
        try {
            // Generate secure filename
            $filename = self::generateAnonymousFilename($validation['extension']);
            $uploadPath = UPLOAD_PATH . 'pages/' . $filename;
            $thumbnailPath = UPLOAD_PATH . 'pages/thumbnails/' . $filename;
            ...
            return [
                ...
                'thumbnail_path' => 'uploads/pages/thumbnails/' . $filename,
                'url' => '/api/v1/files/pages/' . $filename,
                'thumbnail_url' => '/api/v1/files/pages/thumbnails/' . $filename,
                'size' => $validation['size'],
                'dimensions' => $validation['dimensions']
            ];

        } catch (Exception $e) {
            error_log("Page file upload error: " . $e->getMessage());
            return ['success' => false, 'errors' => ['An error occurred during file upload']];
        }
    }

    /**
     * Delete page image and thumbnail
     */
    public static function deletePageImage($filename) {
        if (empty($filename)) {
            return true;
        }
        ...
        $mainPath = UPLOAD_PATH . 'pages/' . $filename;
        if (file_exists($mainPath)) {
            if (!unlink($mainPath)) {
                error_log("Failed to delete page image: {$mainPath}");
                $success = false;
            }
        }
        ...
        if (file_exists($thumbnailPath)) {
            if (!unlink($thumbnailPath)) {
                error_log("Failed to delete page image thumbnail: {$thumbnailPath}");
                $success = false;
            }
        }
    ...
    /**
     * Serve page file securely
     */
    public static function servePageFile($filename, $type = 'main') {
        $filename = basename($filename);

        if ($type === 'thumbnail') {
            $filePath = UPLOAD_PATH . 'pages/thumbnails/' . $filename;
        } else {
            $filePath = UPLOAD_PATH . 'pages/' . $filename;
        }
