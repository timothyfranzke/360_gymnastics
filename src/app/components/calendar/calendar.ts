...
export class CalendarComponent implements OnInit {
  ...
  get daysWithEvents(): { day: number; isToday: boolean; events: CalendarEvent[] }[] {
    if (!this.calendarData?.calendar?.weeks) return [];
    const result: { day: number; isToday: boolean; events: CalendarEvent[] }[] = [];
    for (const week of this.calendarData.calendar.weeks) {
      for (const day of week.days) {
        if (day.isInMonth && day.events.length > 0) {
          result.push({ day: day.day, isToday: day.isToday, events: day.events });
        }
      }
    }
    return result;
