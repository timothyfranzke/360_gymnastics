...
export class Detail implements OnInit, OnDestroy {
      ...
      .subscribe({
        next: (response) => {
          ...
          if (foundClass) {
            ...
            // Load Jackrabbit schedule using jackrabbitId if available, else name
            this.loadJackRabbitSchedule(foundClass.jackrabbitId || foundClass.name);
          }
        ...
        error: (error) => {
          ...
          if (fallbackClass && fallbackClass.id) {
            ...
            // Load Jackrabbit schedule using jackrabbitId from fallback
            this.loadJackRabbitSchedule(fallbackClass.jackrabbitId || fallbackClass.name);
          }
          this.isLoading = false;
        }
      });
  ...
  retryLoadSchedule(): void {
    this.loadJackRabbitSchedule(this.classData.jackrabbitId || this.classData.name);
  }
}